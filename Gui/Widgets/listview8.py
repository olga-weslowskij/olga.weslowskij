...
class ListView8(AbstractView, EventDispatcher):
    ...
    def __init__(self, **kwargs):
        ...
        super(ListView8, self).__init__(**kwargs)
    ...
    def _scroll(self, scroll_y):
        ...
        istart = max(0, (istart)*ncols)
        iend = max(0, (iend )*ncols)

        if istart < self._wstart:
            rstart = max(0, istart - 1 * ncols)
            self.populate(rstart, iend)
            self._wstart = rstart
            self._wend = iend
        elif iend > self._wend:
            rend = min(iend +1 *ncols  , self.adapter.get_count())
            self.populate(istart, rend)
            self._wstart = istart
            self._wend = rend
    ...
    def populate(self, istart=None, iend=None):
        ...
        self.adapter.clear(container.children)

        container.clear_widgets()

        # guess only ?
        if iend is not None:
            #print "Scroll " + str(istart) + " =>" + str(iend) + " " + str(istart - iend)
            # fill with a "padding"
            fh = 0
            for x in xrange(0,istart, ncols):
            ...
            while index < iend:
                ...
                index += 1
                #print "item_view.height " + str(item_view.height)
                container.add_widget(item_view)

            # fill with end padding
            """
            for x in xrange(iend,self._wend, ncols):
                fh += sizes[x] if x in sizes else rh
            container.add_widget(Widget(size_hint_y=None, height=floor(fh)))
            """
            ...
            while available_height > 0:
                for cols in xrange(ncols):
                    item_view = self.adapter.get_view(index)
                    if item_view is None:
                        break

                    sizes[index] = item_view.height
                    #print "item heigth " + str(item_view.height)

                    # thats is set in my widget but doesnt work
                    #item_view.height=rh
                    #item_view.size_hint_y=None
            ...
            if count:
                #container.height = real_height / (count * 1) * self.adapter.get_count()

                if self.row_height is None or self.row_height == 0.0:
                #if self.row_height is None or True:
                    self.row_height = real_height / max((ceil(count/ncols)),1.0)
                    #print "row_height " + str(self.row_height)

                #container.height =  floor(self.adapter.get_count()/8.0) *  self.row_height
                # i wnat a last empty line
                container.height =  (floor(self.adapter.get_count()/ncols*1.0)+1) *  self.row_height
                #print "container.height " + str(container.height)
