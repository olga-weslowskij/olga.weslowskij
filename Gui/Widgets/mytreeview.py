...
class MyTreeView(Widget):
    ...
    def on_touch_down(self, touch):
        ...
        if node.x - self.indent_start <= touch.x < node.x:
            ...
            #node._trigger_update_graphics()
            #node._refresh_text(node.text)
            #node._update_graphics()
            #node._do_layout()
            print node.text
...
if __name__ == '__main__':
    ...
    class TestApp(App):
        ...
        def build(self):
            ...
            add = tv.add_node
            root = add(TreeViewTextInput(text='Level 1, entry 1', is_open=True))
            for x in range(5):
                add(TreeViewTextInput(text='Element %d' % x), root)
            root2 = add(TreeViewTextInput(text='Level 1, entry 2', is_open=False))
            for x in range(24):
                add(TreeViewTextInput(text='Element %d' % x), root2)
            for x in range(5):
                add(TreeViewTextInput(text='Element %d' % x), root)
            root2 = add(TreeViewTextInput(text='Element childs 2', is_open=False),
                        root)
            for x in range(24):
                add(TreeViewTextInput(text='Element %d' % x), root2)
            return tv
